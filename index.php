        ...
        <div class="wrapper">
            <div class="header-flex-box-container">
                    ...
                    <div>
                        ...
                        <?php
                            ...
                            if (isset($_POST['leftadd'])) {
                                $match_id = $_POST['match_id'];
                                if ($current_home_score >= 24 && $current_home_score - $current_away_score >= 1) {
                                    // 主隊達到25分且比客隊多2分
                                    $current_home_score = 0;
                                    $current_away_score = 0;
                                    $home_set = $home_set + 1;
                                ...
                                $match_id = $_POST['match_id'];
                                if ($current_away_score >= 24 && $current_away_score - $current_home_score >= 1) {
                                    // 客隊達到25分且比主隊多2分
                                    $current_home_score = 0;
                                    $current_away_score = 0;
                                    $away_set = $away_set + 1;
                    ...
                    <div>
                        ...
                        <?php
                            ...
                            if(isset($_POST['other_mistake'])){
                                ...
                                if ($conn->query($sql) === TRUE){
                                    $result = $conn->query("SELECT away_error FROM error_substitution_timeout WHERE match_id = '$match_id'");
                                    $row = $result->fetch_assoc();
                                    echo "對方失誤 ".$row['away_error']." 次";
                                }
                                else echo "Error: " . $sql . "<br>" . $conn->error;  
                            }
                            elseif(isset($_POST['substi'])){
                                ...
                                if ($conn->query($sql) === TRUE){
                                    $result = $conn->query("SELECT substitution FROM error_substitution_timeout WHERE match_id = '$match_id'");
                                    $row = $result->fetch_assoc();
                                    echo "換人 ".$row['substitution']." 次";
                                }
                                else echo "Error: " . $sql . "<br>" . $conn->error;
                            }
                            elseif(isset($_POST['timeout'])){
                                ...
                                if ($conn->query($sql) === TRUE){
                                    $result = $conn->query("SELECT time_out FROM error_substitution_timeout WHERE match_id = '$match_id'");
                                    $row = $result->fetch_assoc();
                                    echo "暫停 ".$row['time_out']." 次";
                                }
                                else echo "Error: " . $sql . "<br>" . $conn->error;
                            }
                ...
                <div class="header-box header-box-item-3">
                    <div>
                        <p><span style="font-size: 18px; font-weight: bold;">使用說明</span></p>
                        <p><span style="font-size: 15px;">★ 如果要記錄會得分的行為<br>先將分數+1，再按該行為按鈕<p>
                        <p><span style="font-size: 15px;">★ 系統將根據賽制、比分<br>自動判定比賽是否結束<br>比賽結束將自動跳至首頁<br>可前往「查看我的賽事」查看<p>
                    </div>
                </div>
            </div>
